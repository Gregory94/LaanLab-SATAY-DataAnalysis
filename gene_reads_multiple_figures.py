...
def gene_reads(gene_names=None,region=None,bed_file=None):
...
#%%DEFINE FOR LOOP OVER ENTIRE SCRIPT IF MULTIPLE GENES NEEDS TO BE PROFILED
    if gene_names == None:
        if not isinstance(region[0],list):
            gene_name_list = [region]
        else:
            gene_name_list = region
        use_region = True
    else:
...
#DETERMINE NUMBER OF COLUMNS AND ROWS IN SUBPLOT
    ...
    if number_of_genes == 1:
        ...
        columns = 2
        rows = math.ceil(number_of_genes/columns)
...
#    rvb = mcolors.LinearSegmentedColormap.from_list("", clist)
        ...
        ax = plt.subplot(rows,columns,gene_counter)
...
#    gene_reads(region=['IV',46271,48031],bed_file=r"X:\tnw\BN\LL\Shared\Gregory\Sequence_Alignment_TestData\Michel2017_WT1_SeqData\Cerevisiae_WT1_Michel2017_ProcessedByBenoit\E-MTAB-4885.WT1.bam.bed")
    gene_reads(gene_names=['cdc42','bem1','bem2','bem3','cla4','ste20','rdi1','nrp1'],bed_file=r"X:\tnw\BN\LL\Shared\Gregory\Sequence_Alignment_TestData\Michel2017_WT1_SeqData\Cerevisiae_WT1_Michel2017_ProcessedByBenoit\E-MTAB-4885.WT1.bam.bed")