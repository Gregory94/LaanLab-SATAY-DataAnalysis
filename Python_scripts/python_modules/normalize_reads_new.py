...
#        if dna.Index == len(dna_df2)-1: #FOR THE LAST FEATURE IN DNA_DF2
#            index_closest_nc_region = len(nc_insrt_list)-1 #NOT CORRECT, CURRENTLY TAKES SAME NUMBER OF READS AS THE PREVIOUS FEATURE.
    ...
    del (difference_startposition_featureandnc_list, dna, feature_start_position, index_closest_nc_region, index_count, nc_endposition, nc_startposition, tn_innc_forwardcount, tn_innc_reversecount, nc_endposition_list, nc_insrt_list, tn_innc_bpcount)
    ...
    #!!!INCLUDE FEATURES AFTER LAST NONCODING REGION.
    norm_reads_list = []
    for dna in dna_df2.itertuples():
        if not dna.Feature_name == 'noncoding':
            ...
            print(dna.Feature_name + str(dna.Position[0]))
            norm_reads_list.append(dna.Nreads_truncatedgene * (1/dna.Nbasepairs*0.8) * ((10**6)/(total_reads_in_genome)) * (read_density_chromosome/(window_read_dict.get(dna.Feature_name + str(dna.Position[0])) / window_nclength_dict.get(dna.Feature_name + str(dna.Position[0])))))

    dna_df2['Nreads_truncatedgene_normalized'] = norm_reads_list
    
    return(dna_df2, nc_startposition_list)
